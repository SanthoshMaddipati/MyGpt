...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
    ...
    return (
        <div className={`flex flex-col h-screen min-w72 p-5 dark:bg-gradient-to-b from-[#242124]/30 to-[#000000]/30 border-r border-[#80609F]/30 backdrop-blur-3xl tracking-all duration-500 max-md:absolute left-0 z-1 ${!isMenuOpen && 'max-md:-translate-x-full'}`}>
            {/*logo*/}
            ...
            {/* Search converstion */}
            <div className='flex items-center gap-2 p-3 mt-4 border border-gray-400 dark:border-white/20 round-md'>
            ...
            <div className='flex-1 overflow-y-scroll mt-3 text-sm space-y-3'>
                {
                    chats.filter((chat) =>
                        ...
                        .map((chat) => (
                            <div onClick={()=>{navigate('/');setSelectedChat(chat);setIsMenuOpen(false)}} key={chat._id} className='p-2 px-4 dark:bg-[#57317C]/10 border border-gray-300 dark:border-[#80609F]/15 rounded-md cursor-poinrer flex justify-between group'>
                                <div>
            ...
            <div className='flex items-center justify-between gap-2 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md '>
                ...
                <label className='relative inline-flex cursor-pointer'>
                    ...
                    <div className='w-9 h-4 bg-gray-400 rounded-full peer-checked:bg-purple-600 transition-all'></div>
                    <span className='absolute left-1 top-1 w-3 h-3 bg-white rounded-full transistion-transform peer-checked:translate-x-4'></span>
                </label>
            </div>
