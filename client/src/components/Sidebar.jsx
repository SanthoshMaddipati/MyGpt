...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  ...
  const handleNewChat = async () => {
    if (!user) return toast.error("Login to create chat");
  ...
  return (
    ...
    >
      ...
      {/* User Section */}
      <div className="flex items-center gap-3 p-3 mt-4 border border-gray-300 dark:border-white/15 rounded-md group cursor-pointer">
        <img src={assets.user_icon} className="w-7 rounded-full" alt="" />
        <p className="flex-1 text-sm truncate">{user?.name || "Login"}</p>

        {user && (
          <img
            src={assets.logout_icon}
            onClick={logout}
            className="h-5 hidden group-hover:block dark:invert cursor-pointer"
          />
        )}
      </div>
    </div>
